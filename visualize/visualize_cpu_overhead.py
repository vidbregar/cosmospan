...
VCPU_MAP = {
    'cloud-k3s-master1': 4.0,
    'cloud-k3s-master2': 4.0,
    'cloud-k3s-master3': 4.0,
    'cloud-k3s-pool-workers-worker1': 8.0,
    'cloud-k3s-pool-workers-worker2': 8.0,
    'cloud-k3s-pool-workers-worker3': 8.0,
    'cloud-k3s-pool-workers-worker4': 8.0,
    'cloud-k3s-pool-workers-worker5': 8.0,
    'cloud-k3s-pool-workers-worker6': 8.0,
    'fog-k3s-master1': 8.0,
    'fog-k3s-master2': 8.0,
    'fog-k3s-master3': 8.0,
    'fog-k0s': 8.0
...
def plot_cpu_overhead(baseline_data, proposed_data, output_pdf):
    plt.rcParams.update({
    ...
    for node in ORDERED_NODES:
        if node in baseline_data:
            ...
            idle_series = pd.to_numeric(df_base['%idle'], errors='coerce').dropna()
            avg_idle = idle_series.mean() if not idle_series.empty else 100.0
            cpu_usage_pct = 100.0 - avg_idle
            vcpus = VCPU_MAP.get(node, 1.0)
            baseline_val = (cpu_usage_pct / 100.0) * vcpus * 1000.0
        else:
            baseline_val = np.nan

        if node in proposed_data:
            df_prop = proposed_data[node]
            idle_series = pd.to_numeric(df_prop['%idle'], errors='coerce').dropna()
            avg_idle = idle_series.mean() if not idle_series.empty else 100.0
            cpu_usage_pct = 100.0 - avg_idle
            vcpus = VCPU_MAP.get(node, 1.0)
            proposed_val = (cpu_usage_pct / 100.0) * vcpus * 1000.0
        else:
            proposed_val = np.nan
    ...
    ax.set_ylabel('Avg. vCPU Usage (milli vCPU)', fontweight='bold', labelpad=10, color='#1E293B', y=0.39)
    ax.set_xticks(x)
    ...
    plt.close(fig)
    print(f"Successfully generated CPU overhead visualization PDF: {output_pdf}")
...
def main():
    ...
    output_pdf = 'visualize_cpu_overhead.pdf'

    print("Loading CPU baseline data...")
    baseline_data = get_node_data(baseline_dir, 'cpu')

    print("Loading CPU proposed data...")
    proposed_data = get_node_data(proposed_dir, 'cpu')

    print("Generating CPU overhead visualization...")
    plot_cpu_overhead(baseline_data, proposed_data, output_pdf)
